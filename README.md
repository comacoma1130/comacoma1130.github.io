...
| `size` | 形 | 使用中 |
| --- | --- | --- |
| `"icon"` | アイコンだけの丸ボタン。連続するものは横一列に並ぶ | Instagram / TikTok |
| `"compact"` | 背の低い行。連続するものは 1 つの枠にまとまり、区切り線で仕切られる | LINE スタンプ 3 種 |
| 省略 | 横いっぱいの標準の行（`sub` を付けると 2 行） | — |

`"icon"` は名前が出ないので、`title` が読み上げ用のラベル（`aria-label`）になる。省略しないこと。
